...
func TestAll(t *testing.T) {
	tt := []struct {
		...
		{"positive #1", []int{1, 2}, func(int) bool { return true }, assert.True},
		{"negative #1", []int{1, 2}, func(int) bool { return false }, assert.False},
		{
			"negative #2",
			[]int{1, 2},
			func(i int) bool {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			tc.want(t, all[int](tc.f, tc.items...))
		})
	}
}
