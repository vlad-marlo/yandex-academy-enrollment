...
func TestTimeAlias_MarshalJSON(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			got, err = (*Time)(&tc.time).MarshalJSON()
			require.NoError(t, err)
...
func TestTimeAlias_UnmarshalJSON_Negative(t *testing.T) {
	...
	tt := []struct {
		...
		{"nil data", new(Time), nil, assert.Error},
		{"un-parsable time", new(Time), []byte("\"un-parsable time\""), assert.Error},
	}
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
