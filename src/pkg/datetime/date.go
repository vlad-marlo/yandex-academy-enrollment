...
func (date *Date) Start() time.Time {
	if date == nil {
		return Today().Start()
	}
...
func (date *Date) End() time.Time {
	if date == nil {
		return Today().End()
	}
	return time.Date(date.year, time.Month(date.month), date.day, 23, 59, 59, 0, time.UTC)
}

func (date *Date) Less(other *Date) bool {
	if other == nil {
		return false
	}
	if date == nil {
